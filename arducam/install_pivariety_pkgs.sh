...
if [[ $VERSION_CODENAME == "bullseye" || $VERSION_CODENAME == "buster" ]]; then
    ...
    # bookworm_kernel_version=$(uname -v | grep -oP '\d+\.\d+\.\d+')
    # echo "kernel version is: $bookworm_kernel_version"
    kernel_ver=$(uname -v | grep -oP '\d+\.\d+\.\d+')
    major=$(echo "$kernel_ver" | cut -d. -f1)
    minor=$(echo "$kernel_ver" | cut -d. -f2)

    if [[ $major -eq 6 && $minor -ge 12 ]]; then
        bookworm_kernel_version="${kernel_ver%.*}.y"
    else
        bookworm_kernel_version="$kernel_ver"
    fi

    if [[ $package == *"libcamera"* ]]; then
        if [[ $package == *"apps"* ]]; then
            package='libcamera_apps_'${VERSION_CODENAME}
            package_cfg_name=${package_cfg_names[$package]}
            package_cfg_download_link=${package_cfg_download_links[$package]}
        else
            package='libcamera_'${VERSION_CODENAME}
            package_cfg_name=${package_cfg_names[$package]}
            package_cfg_download_link=${package_cfg_download_links[$package]}
        fi
    else
    ...
    if [[ $package == *"kernel_driver"* ]]; then
        download_link=${package_download_links[$bookworm_kernel_version]}
        pkg_name=${package_names[$bookworm_kernel_version]}
        if [ "$debug_mode" = true ]; then
            echo "download_link"
            echo $download_link
            echo "pkg_name"
    ...
    if [[ $package == *"kernel_driver"* ]]; then
        ...
        sudo apt update
        if [ $package == 'libcamera_'${VERSION_CODENAME} ]; then
            ...
            sudo apt --reinstall install -y ./$pkg_name ./$pkg_name_ipa ./$pkg_name_dev -y
            sudo apt install -y python3-libcamera
            sudo apt install -y python3-picamera2
        elif [ $package == 'libcamera_apps_'${VERSION_CODENAME} ]; then
            if [[ $pkg_name == *.deb ]]; then
                sudo apt --reinstall install -y ./$pkg_name
            else
                apps_deb_path=$(basename $pkg_name .zip)
                unzip ./$pkg_name -d $apps_deb_path
                cd $apps_deb_path
                sudo dpkg -i *.deb
